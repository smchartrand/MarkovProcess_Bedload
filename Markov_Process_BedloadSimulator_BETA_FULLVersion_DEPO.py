...
while Step_2 <= Loops:
    ...
    for n in range(0, Hop_len):
        ...
        if np.sum([Grains_overlap]) == 0:
            ...
            #XNei_array = (np.round(np.linspace(Grains_sort[0, xstart],
#                                                Grains_sort[0, xstop],
#                                                (Grains_sort[0, xstop] -
#                                                 Grains_sort[0, xstart]) /
#                                                 Nei_GS + 1.0), 1))
            ...
            #YNei_array = (np.round(np.arange(Grains_sort[1, ystart], Grains_sort[1, ystop] + 0.1,Nei_GS, dtype = float),2))
            ...
            Elev_Surf = np.zeros((LengthNei1, LengthNei2), dtype=int)
            ...
            for nnn in range(0, Nei_len):
                ...
                X_dist = np.abs(X[mask] - X[0,XNei_Idx])
                Y_dist = np.abs(Y[mask] - Y[YNei_Idx,0])
...
#   plot_step = plot_step + 1
    ...
    # YCenter = np.reshape(CenterCoord[1, :], (1, len(CenterCoord[0])))
    # YCenter = YCenter.reshape(-1)
    xLH = (LxB,HxB,HxB,LxB,LxB)
    yLH = (LyB,LyB,HyB,HyB,LyB)

...
def fu_radcheck(x_Center, y_Center, radius):
    ...
    circ_check = (np.reshape(np.asarray([x_circ_maxck, x_circ_minck, y_circ_maxck, y_circ_minck]), (-1)))
...
def fu_diamrevise(x_Center, y_Center, radius):
    ...
    # existing circle.
    dist_check = ((CenterCoord[0, :] - x_Center) ** 2 + (CenterCoord[1, :] - y_Center) ** 2) ** 0.5
    ...
    if np.sum([radius_overlap]) == 0:
        ...
        radius_temp = dist_check[idx_true] - (Diameter[0, idx_true] / 2)
...
while (AreaTotal / Total_VArea) < Pack:
    ...
    x_Center = XCoordinates[np.unravel_index([idx3], [Length1, Length2], order='F')]
    y_Center = YCoordinates[np.unravel_index([idx3], [Length1, Length2], order='F')]
...
fig.savefig('.\ScriptTest\Images\TotalBedMotions00000.png', format='png', dpi=600)
...
Particle_details = np.vstack((CenterCoord, Diameter, CenterElev))
...
def fu_transport(E_events, BS_boundaries, Particle_details_sort):
    ...
    return n, Entrain_locsx, Entrain_locsy, L_x, T_p, Rand_ELocs_Store, Entrain_diam
###############################################################################
...
while Step_2 <= Loops:
    ...
    # CALL FUNCTION
    ###############
    n, Entrain_locsx, Entrain_locsy, L_x, T_p, Rand_ELocs_Store, Entrain_diam = fu_transport(E_events, BS_boundaries, Particle_details_sort)
    ...
    for n in range(0, SSamp_len):
        Nu_out = np.count_nonzero((Entrain_locsx <= BS_boundaries[n]) & (Hop_locx > BS_boundaries[n]))
        Nu_out_Store.extend([Nu_out])
        if (Step_2 == Loops) & (n == Bed_sampreg-1):
            del Nu_out_Store[0]
    # Deal with particles that cross the downstream boundary
    Hop_locx[Hop_locx > x_max] = 0
    Nu_in = np.count_nonzero(Hop_locx == 0)
...
#   plot_step = plot_step + 1
    ...
    p2 = (PatchCollection(patches, color = "#C1C1C1",
                         alpha=0.9, linewidths=(0, )))#cm.binary
 #   p.set_array(colors)
 #   p.set_clim([5, 950])
 ...
 #   p.set_clim([5, 950])
    ...
    p3 = (PatchCollection(patches, color = "#5B5B5B",
                         alpha=0.9, linewidths=(0, )))#cm.Blues
 #   p1.set_array(colors)
#    p1.set_clim([5, 950])
    ax.add_collection(p3)
    #ax.add_collection(p1)
    plt.show()
    fig.savefig('.\ScriptTest\Images\TotalBedMotions0000%d.png' % plot_step, format='png', dpi=600)
    plot_step = plot_step + 1
    ##
    ##
    # This method of plotting circles comes from Stack Overflow questions\32444037
